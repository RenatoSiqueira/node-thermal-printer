...
class ThermalPrinter {
  ...
  tableCustom(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (obj.align == "CENTER") {
        ...
        if (obj.text != "") this.append(obj.text);
      } else if (obj.align == "CUSTOM") {
        var spaces = (cellWidth - obj.text.toString().length) / 45;
        for (var j = 0; j < spaces; j++) {
          this.append(Buffer.from(" "));
        }
        if (obj.text != "") this.append(obj.text);
  ...
  superSenhas(data) {
    if (data[0].length <= 12) {
      this.setTextSize(2, 2);
      if (data[0] != "") this.append(data[0]);
      var spaces = 12 - data[0].length;
      for (var j = 0; j < spaces; j++) {
        this.append(Buffer.from(" "));
      }
    } else {
      ...
      if (data[0] != "") this.append(data[0]);
      var spaces = 20 - data[0].length;
      for (var j = 0; j < spaces; j++) {
        this.append(Buffer.from(" "));
      }
    }

    if (data[1] != "") this.append(data[1]);
    this.setTextNormal();
    this.newLine();
  }
